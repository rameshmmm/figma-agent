...
server.tool(
  "get_all_frames",
  "Get all frames in the current Figma document",
  {},
  async () => {
    try {
      // First get document info to get current page ID
      const docInfo = await sendCommandToFigma("get_document_info") as {
        currentPage: { id: string; name: string; childCount: number };
      };
      const currentPageId = docInfo.currentPage.id;
      logger.info(`Current page ID: ${currentPageId}`);
      // Then scan for frames
      ...
      // Get detailed info for each frame
      const frameDetails = await Promise.all(
        result.matchingNodes.map(async (frame) => {
          const details = await sendCommandToFigma("get_node_info", { nodeId: frame.id });
          return {
            ...frame,
            details: filterFigmaNode(details)
          };
        })
      );

      logger.info(`Found ${result.count} frames with full details`);
      return {
        content: [
          {
            type: "text",
            text: JSON.stringify(frameDetails, null, 2)
          }
        ]
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error getting all frames info: ${error instanceof Error ? error.message : String(error)}`,
          },
        ],
      };
    }
...
// Search Frames by Description Tool
server.tool(
  "search_frames_by_description",
  "Search for frames in the current document that match a description",
  {
    description: z.string().describe("Description of the frames to search for (e.g., 'bar chart', 'login form')"),
  },
  async ({ description }) => {
    try {
      // First get document info to get current page ID
      ...
      // Search for frames matching the description
      const searchTerms = description.toLowerCase().split(' ');
      const matchingFrames = frameDetails.filter(frame => {
        const frameName = frame.name.toLowerCase();
        const frameText = JSON.stringify(frame.details).toLowerCase();
        return searchTerms.every(term => 
          frameName.includes(term) || frameText.includes(term)
        );
      });
      ...
      return {
        content: [
          {
            type: "text",
            text: `Found ${matchingFrames.length} frames matching "${description}":\n${JSON.stringify(matchingFrames, null, 2)}`
          }
        ]
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error searching frames: ${error instanceof Error ? error.message : String(error)}`
          }
        ]
      };
    }
...
// Create Page with Frames Tool
server.tool(
  "create_page_with_frames",
  "Create a new page with selected frames arranged in a grid",
  {
    frameIds: z.array(z.string()).describe("Array of frame IDs to include in the new page"),
    layout: z.enum(["horizontal", "vertical", "grid"]).describe("Layout arrangement for the frames"),
    spacing: z.number().optional().describe("Spacing between frames in pixels").default(40),
    pageName: z.string().optional().describe("Name for the new page").default("New Page")
  },
  async ({ frameIds, layout, spacing, pageName }) => {
    try {
      // Create a new frame to hold all the frames
      const containerFrame = await sendCommandToFigma("create_frame", {
        x: 0,
        y: 0,
        width: 1200,
        height: 800,
        name: pageName,
        layoutMode: layout === "grid" ? "HORIZONTAL" : layout === "horizontal" ? "HORIZONTAL" : "VERTICAL",
      ...
      // Clone each frame and add it to the container
      const clonedFrames = await Promise.all(
        frameIds.map(async (frameId, index) => {
          const clone = await sendCommandToFigma("clone_node", { nodeId: frameId }) as { id: string; name: string };
          
          // Set the clone's parent to our container frame
      ...
      return {
        content: [
          {
            type: "text",
            text: `Created new page "${pageName}" with ${clonedFrames.length} frames arranged in ${layout} layout.\nContainer frame ID: ${containerFrame.id}\nCloned frames: ${JSON.stringify(clonedFrames, null, 2)}`
          }
        ]
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error creating page with frames: ${error instanceof Error ? error.message : String(error)}`
          }
        ]
      };
    }
...
server.tool(
  "create_bar_chart",
  "Create a bar chart frame with dummy data",
  {
    x: z.number().describe("X position for the chart"),
    y: z.number().describe("Y position for the chart"),
    width: z.number().describe("Width of the chart frame"),
    height: z.number().describe("Height of the chart frame"),
    title: z.string().describe("Title of the chart"),
    data: z.array(z.object({
      label: z.string(),
      value: z.number()
    })).optional().describe("Optional data for the chart. If not provided, random dummy data will be generated"),
    barColor: z.object({
      r: z.number().min(0).max(1),
      g: z.number().min(0).max(1),
      b: z.number().min(0).max(1),
      a: z.number().min(0).max(1).optional()
    }).optional().describe("Color for the bars in RGBA format"),
    maxValue: z.number().optional().describe("Maximum value for the y-axis. If not provided, will be calculated from data")
  },
  async ({ x, y, width, height, title, data, barColor, maxValue }) => {
    try {
      // Generate dummy data if not provided
      const chartData = data || [
        { label: "Jan", value: Math.random() * 100 },
        { label: "Feb", value: Math.random() * 100 },
        { label: "Mar", value: Math.random() * 100 },
        { label: "Apr", value: Math.random() * 100 },
        { label: "May", value: Math.random() * 100 },
        { label: "Jun", value: Math.random() * 100 }
      ];

      const maxBarValue = maxValue || Math.max(...chartData.map(d => d.value)) * 1.2;
      const barWidth = (width - 100) / chartData.length;
      const barSpacing = barWidth * 0.2;
      const actualBarWidth = barWidth - barSpacing;
      const chartHeight = height - 100; // Leave space for title and labels

      // Create main chart frame
      const chartFrame = await sendCommandToFigma("create_frame", {
        x,
        y,
        width,
        height,
      ...
      for (let i = 0; i < chartData.length; i++) {
        const barHeight = (chartData[i].value / maxBarValue) * chartHeight;
        const barX = x + 50 + (i * barWidth);
        const barY = y + height - 50 - barHeight;

        // Create bar
        await sendCommandToFigma("create_rectangle", {
          x: barX,
          y: barY,
          width: actualBarWidth,
          height: barHeight,
          name: `Bar ${chartData[i].label}`,
          parentId: chartFrame.id,
          fillColor: barColor || { r: 0.2, g: 0.4, b: 0.8, a: 1 }
        });
        ...
        await sendCommandToFigma("create_text", {
          x: barX,
          y: barY - 20,
          text: Math.round(chartData[i].value).toString(),
          fontSize: 12,
          parentId: chartFrame.id
        });
      }
      ...
      return {
        content: [
          {
            type: "text",
            text: `Created bar chart "${title}" with ${chartData.length} bars. Frame ID: ${chartFrame.id}`
          }
        ]
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error creating bar chart: ${error instanceof Error ? error.message : String(error)}`
          }
        ]
      };
    }
...
// Create Page with Bar Charts Tool
server.tool(
  "create_page_with_bar_charts",
  "Create a new page with two bar charts with dummy data",
  {
    pageName: z.string().optional().describe("Name for the new page").default("Bar Charts Page"),
    chart1Title: z.string().optional().describe("Title for the first chart").default("Monthly Revenue"),
    chart2Title: z.string().optional().describe("Title for the second chart").default("Monthly Expenses"),
    layout: z.enum(["horizontal", "vertical"]).optional().describe("Layout arrangement for the charts").default("horizontal"),
    spacing: z.number().optional().describe("Spacing between charts in pixels").default(40)
  },
  async ({ pageName, chart1Title, chart2Title, layout, spacing }) => {
    try {
      // Create a container frame for the page
      const containerFrame = await sendCommandToFigma("create_frame", {
        x: 0,
        y: 0,
        width: layout === "horizontal" ? 1600 : 800,
        height: layout === "horizontal" ? 800 : 1600,
        name: pageName,
      ...
      // Create first chart
      const chart1 = await sendCommandToFigma("create_bar_chart", {
        x: layout === "horizontal" ? spacing : spacing,
        y: layout === "horizontal" ? spacing : spacing,
        width: layout === "horizontal" ? 700 : 700,
        height: layout === "horizontal" ? 600 : 600,
        title: chart1Title,
        barColor: { r: 0.2, g: 0.4, b: 0.8, a: 1 }
      }) as { id: string };

      // Create second chart
      const chart2 = await sendCommandToFigma("create_bar_chart", {
        x: layout === "horizontal" ? 800 + spacing : spacing,
        y: layout === "horizontal" ? spacing : 700 + spacing,
        width: layout === "horizontal" ? 700 : 700,
        height: layout === "horizontal" ? 600 : 600,
        title: chart2Title,
        barColor: { r: 0.8, g: 0.2, b: 0.2, a: 1 }
      }) as { id: string };

      // Move charts into container
      await sendCommandToFigma("move_node", {
        nodeId: chart1.id,
        x: 0,
        y: 0
      ...
      await sendCommandToFigma("move_node", {
        nodeId: chart2.id,
        x: layout === "horizontal" ? 800 : 0,
        y: layout === "horizontal" ? 0 : 700
      });
      ...
      return {
        content: [
          {
            type: "text",
            text: `Created page "${pageName}" with two bar charts:\n1. ${chart1Title} (ID: ${chart1.id})\n2. ${chart2Title} (ID: ${chart2.id})`
          }
        ]
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error creating page with bar charts: ${error instanceof Error ? error.message : String(error)}`
          }
        ]
      };
    }
...
server.tool(
  "select_frame",
  "Select a frame in Figma by its ID",
  {
    frameId: z.string().describe("The ID of the frame to select")
  },
  async ({ frameId }) => {
    try {
      console.log("select_frame", frameId);
      const result = await sendCommandToFigma("select_frame", { frameId: frameId });
        ...
        content: [
          {
            type: "text",
            text: typedResult.success 
              ? `Successfully selected frame "${typedResult.name}"`
              : `Failed to select frame with ID: ${frameId}`
          }
        ]
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error selecting frame: ${error instanceof Error ? error.message : String(error)}`
          }
        ]
      };
    }
...
type FigmaCommand =
  ...
  | "select_frame"
  | "update_frame"
  | "update_frame_with_same_data";

type CommandParams = {
  get_document_info: Record<string, never>;
  get_all_frames: Record<string, never>;
...
// Add this type definition before the update_frame tool
type FigmaNodeInfo = {
  id: string;
  name: string;
  type: string;
...
server.tool(
  "update_frame",
  "Update properties of an existing frame in Figma",
  {
    frameId: z.string().describe("The ID of the frame to update"),
    updates: z.object({
      name: z.string().optional().describe("New name for the frame"),
      width: z.number().positive().optional().describe("New width for the frame"),
      height: z.number().positive().optional().describe("New height for the frame"),
      x: z.number().optional().describe("New X position for the frame"),
      y: z.number().optional().describe("New Y position for the frame"),
      fillColor: z.object({
        r: z.number().min(0).max(1),
        g: z.number().min(0).max(1),
        b: z.number().min(0).max(1),
        a: z.number().min(0).max(1).optional()
      }).optional().describe("New fill color in RGBA format"),
      layoutMode: z.enum(["NONE", "HORIZONTAL", "VERTICAL"]).optional().describe("New layout mode for the frame"),
      layoutWrap: z.enum(["NO_WRAP", "WRAP"]).optional().describe("New layout wrap behavior for the frame"),
      itemSpacing: z.number().optional().describe("New spacing between items in the frame"),
      paddingTop: z.number().optional().describe("New top padding for the frame"),
      paddingRight: z.number().optional().describe("New right padding for the frame"),
      paddingBottom: z.number().optional().describe("New bottom padding for the frame"),
      paddingLeft: z.number().optional().describe("New left padding for the frame"),
      primaryAxisAlignItems: z.enum(["MIN", "MAX", "CENTER", "SPACE_BETWEEN"]).optional().describe("New primary axis alignment for the frame"),
      counterAxisAlignItems: z.enum(["MIN", "MAX", "CENTER", "BASELINE"]).optional().describe("New counter axis alignment for the frame")
    }).describe("Object containing the properties to update")
  },
  async ({ frameId, updates }) => {
    try {
      // Get current frame info to verify it exists and compare values
      const currentInfo = await sendCommandToFigma("get_node_info", { nodeId: frameId }) as FigmaNodeInfo;
      
      // Track which updates would actually change something
      const effectiveUpdates: string[] = [];
      const updatesToApply: any = {};

      // Check each update property and only apply if it would change something
      if (updates.name !== undefined && updates.name !== currentInfo.name) {
        updatesToApply.name = updates.name;
        effectiveUpdates.push(`name from "${currentInfo.name}" to "${updates.name}"`);
      }

      if (updates.width !== undefined && updates.width !== currentInfo.width) {
        updatesToApply.width = updates.width;
        effectiveUpdates.push(`width from ${currentInfo.width} to ${updates.width}`);
      }

      if (updates.height !== undefined && updates.height !== currentInfo.height) {
        updatesToApply.height = updates.height;
        effectiveUpdates.push(`height from ${currentInfo.height} to ${updates.height}`);
      }

      if (updates.x !== undefined && updates.x !== currentInfo.x) {
        updatesToApply.x = updates.x;
        effectiveUpdates.push(`x position from ${currentInfo.x} to ${updates.x}`);
      }

      if (updates.y !== undefined && updates.y !== currentInfo.y) {
        updatesToApply.y = updates.y;
        effectiveUpdates.push(`y position from ${currentInfo.y} to ${updates.y}`);
      }

      // For other properties, we'll need to get more detailed info
      const detailedInfo = await sendCommandToFigma("get_node_info", { nodeId: frameId });
      
      // Check fill color
      if (updates.fillColor !== undefined) {
        const currentFill = (detailedInfo as any).fills?.[0]?.color;
        if (currentFill && (
          currentFill.r !== updates.fillColor.r ||
          currentFill.g !== updates.fillColor.g ||
          currentFill.b !== updates.fillColor.b ||
          (updates.fillColor.a !== undefined && currentFill.a !== updates.fillColor.a)
        )) {
          updatesToApply.fillColor = updates.fillColor;
          effectiveUpdates.push("fill color");
        }
      }

      // Check layout properties
      if (updates.layoutMode !== undefined && (detailedInfo as any).layoutMode !== updates.layoutMode) {
        updatesToApply.layoutMode = updates.layoutMode;
        effectiveUpdates.push(`layout mode from ${(detailedInfo as any).layoutMode || 'NONE'} to ${updates.layoutMode}`);
      }

      if (updates.layoutWrap !== undefined && (detailedInfo as any).layoutWrap !== updates.layoutWrap) {
        updatesToApply.layoutWrap = updates.layoutWrap;
        effectiveUpdates.push(`layout wrap from ${(detailedInfo as any).layoutWrap || 'NO_WRAP'} to ${updates.layoutWrap}`);
      }

      // Check padding
      if (updates.paddingTop !== undefined && (detailedInfo as any).paddingTop !== updates.paddingTop) {
        updatesToApply.paddingTop = updates.paddingTop;
        effectiveUpdates.push(`top padding from ${(detailedInfo as any).paddingTop || 0} to ${updates.paddingTop}`);
      }

      if (updates.paddingRight !== undefined && (detailedInfo as any).paddingRight !== updates.paddingRight) {
        updatesToApply.paddingRight = updates.paddingRight;
        effectiveUpdates.push(`right padding from ${(detailedInfo as any).paddingRight || 0} to ${updates.paddingRight}`);
      }

      if (updates.paddingBottom !== undefined && (detailedInfo as any).paddingBottom !== updates.paddingBottom) {
        updatesToApply.paddingBottom = updates.paddingBottom;
        effectiveUpdates.push(`bottom padding from ${(detailedInfo as any).paddingBottom || 0} to ${updates.paddingBottom}`);
      }

      if (updates.paddingLeft !== undefined && (detailedInfo as any).paddingLeft !== updates.paddingLeft) {
        updatesToApply.paddingLeft = updates.paddingLeft;
        effectiveUpdates.push(`left padding from ${(detailedInfo as any).paddingLeft || 0} to ${updates.paddingLeft}`);
      }

      // Check alignment
      if (updates.primaryAxisAlignItems !== undefined && (detailedInfo as any).primaryAxisAlignItems !== updates.primaryAxisAlignItems) {
        updatesToApply.primaryAxisAlignItems = updates.primaryAxisAlignItems;
        effectiveUpdates.push(`primary axis alignment from ${(detailedInfo as any).primaryAxisAlignItems || 'MIN'} to ${updates.primaryAxisAlignItems}`);
      }

      if (updates.counterAxisAlignItems !== undefined && (detailedInfo as any).counterAxisAlignItems !== updates.counterAxisAlignItems) {
        updatesToApply.counterAxisAlignItems = updates.counterAxisAlignItems;
        effectiveUpdates.push(`counter axis alignment from ${(detailedInfo as any).counterAxisAlignItems || 'MIN'} to ${updates.counterAxisAlignItems}`);
      }

      // Check item spacing
      if (updates.itemSpacing !== undefined && (detailedInfo as any).itemSpacing !== updates.itemSpacing) {
        updatesToApply.itemSpacing = updates.itemSpacing;
        effectiveUpdates.push(`item spacing from ${(detailedInfo as any).itemSpacing || 0} to ${updates.itemSpacing}`);
      }

      // If no effective updates, return early
      if (effectiveUpdates.length === 0) {
        return {
          content: [
            {
              type: "text",
              text: `No changes needed for frame "${currentInfo.name}" (ID: ${frameId}) - all values are already set as requested.`
            }
          ]
        };
      }

      // Apply only the updates that would change something
      if (updatesToApply.name !== undefined) {
        await sendCommandToFigma("set_text_content", { nodeId: frameId, text: updatesToApply.name });
      }

      if (updatesToApply.width !== undefined || updatesToApply.height !== undefined) {
        await sendCommandToFigma("resize_node", {
          nodeId: frameId,
          width: updatesToApply.width || currentInfo.width,
          height: updatesToApply.height || currentInfo.height
        });
      }

      if (updatesToApply.x !== undefined || updatesToApply.y !== undefined) {
        await sendCommandToFigma("move_node", {
          nodeId: frameId,
          x: updatesToApply.x || currentInfo.x,
          y: updatesToApply.y || currentInfo.y
        });
      }

      if (updatesToApply.fillColor !== undefined) {
        await sendCommandToFigma("set_fill_color", {
          nodeId: frameId,
          r: updatesToApply.fillColor.r,
          g: updatesToApply.fillColor.g,
          b: updatesToApply.fillColor.b,
          a: updatesToApply.fillColor.a
        });
      }

      if (updatesToApply.layoutMode !== undefined || updatesToApply.layoutWrap !== undefined) {
        await sendCommandToFigma("set_layout_mode", {
          nodeId: frameId,
          layoutMode: updatesToApply.layoutMode || "NONE",
          layoutWrap: updatesToApply.layoutWrap
        });
      }

      if (updatesToApply.paddingTop !== undefined || updatesToApply.paddingRight !== undefined || 
          updatesToApply.paddingBottom !== undefined || updatesToApply.paddingLeft !== undefined) {
        await sendCommandToFigma("set_padding", {
          nodeId: frameId,
          paddingTop: updatesToApply.paddingTop,
          paddingRight: updatesToApply.paddingRight,
          paddingBottom: updatesToApply.paddingBottom,
          paddingLeft: updatesToApply.paddingLeft
        });
      }

      if (updatesToApply.primaryAxisAlignItems !== undefined || updatesToApply.counterAxisAlignItems !== undefined) {
        await sendCommandToFigma("set_axis_align", {
          nodeId: frameId,
          primaryAxisAlignItems: updatesToApply.primaryAxisAlignItems,
          counterAxisAlignItems: updatesToApply.counterAxisAlignItems
        });
      }

      if (updatesToApply.itemSpacing !== undefined) {
        await sendCommandToFigma("set_item_spacing", {
          nodeId: frameId,
          itemSpacing: updatesToApply.itemSpacing
        });
      }
      ...
      return {
        content: [
          {
            type: "text",
            text: `Updated frame "${currentInfo.name}" (ID: ${frameId}):\n${effectiveUpdates.map(update => `- Changed ${update}`).join('\n')}`
          }
        ]
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error updating frame: ${error instanceof Error ? error.message : String(error)}`
          }
        ]
      };
    }
...
// Update Frame with Same Data Tool
server.tool(
  "update_frame_with_same_data",
  "Update a frame with its current data (useful for refreshing the frame)",
  {
    frameId: z.string().describe("The ID of the frame to update with its current data")
  },
  async ({ frameId }) => {
    try {
      ...
      // Create updates object with current data
      const updates = {
        ...
        x: currentInfo.x,
        y: currentInfo.y,
        // Note: We don't include layout properties or colors as they might not be available in the basic node info
      };

      // Apply the update with the same data
      const result = await sendCommandToFigma("update_frame", {
        frameId,
        updates
      });
      ...
      return {
        content: [
          {
            type: "text",
            text: `Updated frame "${currentInfo.name}" (ID: ${frameId}) with its current data:\n${JSON.stringify(updates, null, 2)}`
          }
        ]
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error updating frame with same data: ${error instanceof Error ? error.message : String(error)}`
          }
        ]
      };
    }
